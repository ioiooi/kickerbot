const lookingForMoreMessage = (text, playerArray) => {
  const attachments = [
    {
      fallback: 'Oops. That was not supposed to happen.',
      color: 'good',
      text: 'Looking for more',
      callback_id: 'kicker_join',
...
const lfmOrGameReady = (time, players) => {
  if (players.length < 4) {
    return lookingForMoreMessage(`<!channel> kicker ${time}?`, players);
  }

  return gameReadyMessage(`kicker ${time}?`, players.slice(0, 4));
};

const leaveMessage = (channel, ts, original_message) => {
  return [
    {
      fallback: 'Oops. That was not supposed to happen.',
      text: 'Leave the game lobby.',
      callback_id: 'kicker_leave',
      actions: [
        {
          name: 'kicker_out',
          text: 'Leave',
          type: 'button',
          value: JSON.stringify({ channel, ts, original_message })
        }
      ]
    }
  ];
...
const deleteGameMessage = (channel, ts) => {
  return [
    {
      fallback: 'Oops. That was not supposed to happen.',
